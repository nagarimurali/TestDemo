...
foreach ($row in $inputData) {
    ...
    for ($i = 0; $i -lt $codeParts.Length; $i++) {
        ...
        $name = $inputData | Where-Object { $_.Code -eq $code } | Select-Object -ExpandProperty Name
        $currentHierarchy["Level$i"] = "$name ($code)"
    }
    ...
    if ($isFirstRow) {
        $outputObject | Add-Member -MemberType NoteProperty -Name "Term Set Name" -Value "PBS"
        $isFirstRow = $false
    } else {
        $outputObject | Add-Member -MemberType NoteProperty -Name "Term Set Name" -Value ""
    }

    $outputObject | Add-Member -MemberType NoteProperty -Name "Term Set Description" -Value ""
    $outputObject | Add-Member -MemberType NoteProperty -Name "LCID" -Value ""
    $outputObject | Add-Member -MemberType NoteProperty -Name "Available for Tagging" -Value "TRUE"
    $outputObject | Add-Member -MemberType NoteProperty -Name "Term Description" -Value ""

    for ($j = 0; $j -le $global:highestValue; $j++) {
        $levelKey = "Level$($j + 1) Term"
